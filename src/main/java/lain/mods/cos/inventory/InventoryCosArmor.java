...
public class InventoryCosArmor implements IInventory {
    ...
    public ItemStack decrStackSize(int slot, int num) {
        if (stacks == null || slot < 0 || slot >= stacks.length) return null;

        ItemStack stack = stacks[slot];

        if (stack == null) return null;

        if (stack.stackSize <= num) stacks[slot] = null;
        else stack = stack.splitStack(num);
    ...
    public boolean isSkinArmor(int slot) {
        if (isSkinArmor == null || slot < 0 || slot >= isSkinArmor.length) return false;
    ...
    public void setInventorySlotContents(int slot, ItemStack stack) {
        if (stacks == null || slot < 0 || slot >= stacks.length) return;
    ...
    public void setSkinArmor(int slot, boolean enabled) {
        if (isSkinArmor == null || slot < 0 || slot >= isSkinArmor.length) return;
    ...
    public void writeToNBT(NBTTagCompound compound) {
        ...
        for (int i = 0; i < stacks.length; i++) {
            ...
            if (stacks[i] != null) stacks[i].writeToNBT(invSlot);
